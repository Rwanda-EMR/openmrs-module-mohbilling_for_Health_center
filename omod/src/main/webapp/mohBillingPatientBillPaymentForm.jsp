...
<script type="text/javascript">
	$j(function () {
		...
		var insuId = $j('#InsuranceId').val();
		function calculateSum() {
			var $checked = $j('.items:checked');
			total = 0.0;
			$checked.each(function () {
				var id=$j(this).val();
				var servId = $j('#serviceId_'+id).val();
				var paidQty = $j('#paidQty_'+id).val();
				var up=$j('#up_'+id).val();
				var cost=paidQty*up;

				<!-- if((insuId==60) && (servId==26)){ -->
				if((insuId==="MUTUELLE_CBHI_90%") && (servId==="AMBULANCE")){
					total += parseFloat(cost*0.1);
				}
				else if(((insuId==="MUTUELLE_CBHI_90%") && (servId==="AUTRES")) || ((insuId==="MUTUELLE_CBHI_90%") && (servId==="TICKET MODERATEUR"))){
					total += parseFloat(cost);
				}
				else if (insuId==="MUTUELLE_CBHI_90%"){
					const tm = 0;
					total += parseFloat(tm);
				}
				else if ((insuId==="MUTUELLE_CBHI_100%" && (servId==="AUTRES"))){
					total += parseFloat(cost);
				}
				else {
					if(!isNaN(cost))
	...
	<form action="patientBillPayment.form?consommationId=${consommation.consommationId}&ipCardNumber=${param.ipCardNumber}&save=true" method="post" id="formSaveBillPayment">
		<table width="99%">
			<tr>
				...
				<th class="columnHeader">Service Category</th>
			...
			<c:forEach items="${consommation.billItems}" var="billItem" varStatus="status">
				...
				<c:if test="${not billItem.voided}">
					<tr>
						...
						<td class="rowValue right ${(status.count%2!=0)?'even':''}"><input type="text"  size="10" id="serviceId_${billItem.patientServiceBillId}"  value="${billItem.hopService.name}" style="border:none; text-align: center;"/></td>
			...
			<tr>
				<td colspan="7">
					<input type="hidden" id="InsuranceId" value="${consommation.beneficiary.insurancePolicy.insurance.name}"/>
					<p align="center" style="color: red; " id="tot"></p></td>
