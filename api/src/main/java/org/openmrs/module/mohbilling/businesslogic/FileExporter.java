...
public class FileExporter {
	...
	public void printPayment(HttpServletRequest request,HttpServletResponse response, BillPayment payment,Consommation consommation,String filename) throws Exception{
		...
		try {
			...
			document.add(new Paragraph("............................................................................................................................................................\n"));
			displayAllItems(document,payment, consommation, fontselector);
			document.add(new Paragraph("\n"));
...
*/
		...
		*/
		...
		BigDecimal totalDueAmountAT = new BigDecimal(0);
		BigDecimal totalDueAmount = new BigDecimal(0);
		BigDecimal totalDueAmountAMB = new BigDecimal(0);
		for (PatientServiceBill item : consommation.getBillItems())
		{
			if(!item.isVoided()) {
				if ((item.getConsommation().getBeneficiary().getInsurancePolicy().getInsurance().getName().contains("MUTUELLE_CBHI_90%"))){
					if (item.getHopService().getName().contains("AUTRES") || item.getHopService().getName().contains("TICKET MODERATEUR")){
						totalDueAmountAT =totalDueAmountAT.add(item.getQuantity().multiply(item.getUnitPrice()));
					}
					if (item.getHopService().getName().contains("AMBULANCE")){
						totalDueAmountAMB = totalDueAmountAMB.add(item.getQuantity().multiply(item.getUnitPrice()).multiply(patientRate).divide(new BigDecimal(100)));
					}
					totalDueAmount = totalDueAmountAT.add(totalDueAmountAMB);
				}
				else {
					totalDueAmount = totalDueAmount.add(item.getQuantity().multiply(item.getUnitPrice()).multiply(patientRate).divide(new BigDecimal(100)));
				}
			}
		}
	...
	public void displayAllItems(Document document,BillPayment payment,Consommation consommation,FontSelector fontSelector) throws DocumentException {
		NumberFormat formatter = new DecimalFormat("#0.0");
		NumberFormat fm = new DecimalFormat("#0");
		InsurancePolicy ip = consommation.getBeneficiary().getInsurancePolicy();
		Float insuranceRate = ip.getInsurance().getCurrentRate().getRate();
...
//		c = new PdfPCell(fontSelector.process(""+consommation.getPatientBill().getAmount()));
	...
	public void displayServiceRevenues(Document document,GlobalBill gb,List<ServiceRevenue> sr,FontSelector fontSelector) throws DocumentException {
		...
		if (gb.getAdmission().getInsurancePolicy().getInsurance().getName().contains("MUTUELLE_CBHI_90%")){
			cell = new PdfPCell(boldFont.process("TOT(100%)"));
			table.addCell(cell);

			cell = new PdfPCell(boldFont.process("TOT INS.- TM(200RFW)"));
			table.addCell(cell);

			cell = new PdfPCell(boldFont.process("TM(200RFW)"));
			table.addCell(cell);
		}
		else {
			cell = new PdfPCell(boldFont.process("TOT(100%)"));
			...
			cell = new PdfPCell(boldFont.process("TOT INS.("+insuranceRate+"%)"));
			table.addCell(cell);

			cell = new PdfPCell(boldFont.process("TOT PAT.("+patientRate+"%)"));
			table.addCell(cell);
		}
		NumberFormat formatter = new DecimalFormat("#0.00");
		BigDecimal totalBill = new BigDecimal(0);
		BigDecimal totalBillAmbulance = new BigDecimal(0);
		BigDecimal totalBillAmbulanceClient = new BigDecimal(0);
		BigDecimal totalCBHI = new BigDecimal(0);
		BigDecimal totalBillOther = new BigDecimal(0);
			...
			if (r != null && r.getBillItems().size() != 0) {
				...
				for (PatientServiceBill item : r.getBillItems()) {
					...
					if (!item.isVoided()) {
						...
						cell = new PdfPCell(fontSelector.process("" + item.getUnitPrice()));
						table.addCell(cell);
						hopService = item.getHopService().getName();
						BigDecimal qua = item.getQuantity();
						BigDecimal up = item.getUnitPrice();
						total=qua.multiply(up);
						if (item.getConsommation().getBeneficiary().getInsurancePolicy().getInsurance().getName().contains("MUTUELLE_CBHI_90%")) {
							if (item.getHopService().getName().contains("AMBULANCE")) {
								totalBillAmbulance = totalBillAmbulance.add(qua.multiply(up).multiply(new BigDecimal(insuranceRate).divide(new BigDecimal(100))));
								totalBillAmbulanceClient =  totalBillAmbulanceClient.add(qua.multiply(up).multiply(new BigDecimal(patientRate).divide(new BigDecimal(100))));

							}
							else {
								totalBillOther = totalBillOther.add(qua.multiply(up));
							}
							totalCBHI = new BigDecimal(0);
							}
						else {
						...
						if (item.getConsommation().getBeneficiary().getInsurancePolicy().getInsurance().getName().contains("MUTUELLE_CBHI_90%")) {
							if (item.getHopService().getName().contains("AMBULANCE")) {
								...
								cell = new PdfPCell(fontSelector.process("" + formatter.format(total)));
								table.addCell(cell);
								up = new BigDecimal(0.0);
								total =qua.multiply(up);
								cell = new PdfPCell(fontSelector.process("" + formatter.format(total)));
								table.addCell(cell);
								qua = item.getQuantity();
								up = item.getUnitPrice();
								total=qua.multiply(up);
								totalByCategory = totalByCategory.add(total);
							}
						} else {
				...
				table.addCell(c);
				if (gb.getAdmission().getInsurancePolicy().getInsurance().getName().contains("MUTUELLE_CBHI_90%")) {
					if (!hopService.contains ("AMBULANCE")){
						c = new PdfPCell(boldFont.process("" + formatter.format(totalByCategory)));
						table.addCell(c);
						c = new PdfPCell(boldFont.process("" + formatter.format(totalByCategory)));
						table.addCell(c);
						c = new PdfPCell(boldFont.process("" + formatter.format(0)));
						table.addCell(c);
					}
					else {
						c = new PdfPCell(boldFont.process("" + formatter.format(totalByCategory)));
		...
		if(gb.getAdmission().getInsurancePolicy().getInsurance().getName().contains("MUTUELLE_CBHI_90%")){
				...
				table.addCell(c);
				c = new PdfPCell(boldFont.process(""+formatter.format(totalBillOther.add(totalBillAmbulance).subtract(new BigDecimal(200)))));
				table.addCell(c);
			    c = new PdfPCell(boldFont.process(""+formatter.format(totalCBHI.add((totalBillAmbulanceClient)).add(new BigDecimal(200)))));
				table.addCell(c);}
		else {
			c = new PdfPCell(boldFont.process("" + formatter.format(totalBill)));
			table.addCell(c);
	...
	public void displayDepartRevenues(Document document,List<DepartmentRevenues> departRevenues,FontSelector fontSelector) throws DocumentException {
		...
		for (DepartmentRevenues dr : departRevenues) {
			...
			for (PaidServiceRevenue psr : dr.getPaidServiceRevenues()) {
				...
				BigDecimal amountPaid = psr.getPaidAmount().round(mc);
				cell = new PdfPCell(fontSelector.process(""+amountPaid));
	...
	public static void exportData(HttpServletRequest request, HttpServletResponse response, Insurance insurance, List<String> columns,List<AllServicesRevenue> listOfAllServicesRevenue)throws Exception,
		...
		op.print(",100%");
		op.print(","+insRate+"%");
		if(insurance.getName().contains("MUTUELLE_CBHI_90%")){
			op.println(","+"TM");
		}
		else {
		...
		for (AllServicesRevenue asr : listOfAllServicesRevenue) {
			Float patientDue=0f;
			Float patientDueAmbulance =0f;
			Float patientDueNoAmbulance=0f;
			Float insuranceDue =0f;
			Float insuranceDueAmbulance =0f;
			Float insuranceDueNoAmbulance =0f;
			String ambulance =null;
			Float allDueAmount = 0f;

			Consommation c = asr.getConsommation();
			Float insuranceRate = asr.getConsommation().getBeneficiary().getInsurancePolicy().getInsurance().getCurrentRate().getRate();
			if(asr.getConsommation().getBeneficiary().getInsurancePolicy().getInsurance().getName().contains("MUTUELLE_CBHI_90%")){
				for (ServiceRevenue serviceRev : asr.getRevenues()){
					for (PatientServiceBill psb : serviceRev.getBillItems()){
						ambulance = psb.getHopService().getName();

						if (!psb.isVoided()){
							if (ambulance.contains("AMBULANCE")){
								insuranceDueAmbulance = (insuranceDueAmbulance + psb.getQuantity().multiply(psb.getUnitPrice()).floatValue())*insuranceRate/100;
								patientDueAmbulance  = ((patientDueAmbulance + psb.getQuantity().multiply(psb.getUnitPrice()).floatValue())*(100-insuranceRate)/100);
							}
							if (!psb.getHopService().getName().contains("AMBULANCE")){
								insuranceDueNoAmbulance = (insuranceDueNoAmbulance + psb.getQuantity().multiply(psb.getUnitPrice()).floatValue());
								patientDueNoAmbulance=0f;
							}
							allDueAmount = (allDueAmount + psb.getQuantity().multiply(psb.getUnitPrice()).floatValue());
						}
					}
				}
			}
			else {
				insuranceDue = asr.getAllDueAmounts().floatValue()*insuranceRate/100;
				patientDue= asr.getAllDueAmounts().floatValue()*((100-insuranceRate)/100);
			}
			if(asr.getConsommation().getBeneficiary().getInsurancePolicy().getInsurance().getName().contains("MUTUELLE_CBHI_90%")){
				insuranceDue = insuranceDueAmbulance + insuranceDueNoAmbulance -200;
				patientDue = patientDueAmbulance + patientDueNoAmbulance + 200;
			}
			i++;
			...
			if (asr.getConsommation().getBeneficiary().getInsurancePolicy().getInsurance().getName().contains("MUTUELLE_CBHI_90%")){
				op.print("," + ReportsUtil.roundTwoDecimals(allDueAmount) + "," + ReportsUtil.roundTwoDecimals(insuranceDue) + "," + ReportsUtil.roundTwoDecimals(patientDue));
			}
			else {
