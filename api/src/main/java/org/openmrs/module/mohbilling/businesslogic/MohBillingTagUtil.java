...
//		MathContext mc = new MathContext(BigDecimal.ROUND_HALF_DOWN);
		...
		if (null == consommationId)
			...
			try {
				...
				if(consomm.getBeneficiary().getInsurancePolicy().getInsurance().getName().contains("MUTUELLE_CBHI_100%") || consomm.getBeneficiary().getInsurancePolicy().getInsurance().getName().contains("MUTUELLE_CBHI_90%")){
					amountNotPaid=0d;
				}

				else if (consomm.getBeneficiary().getInsurancePolicy().getThirdParty() == null) {
