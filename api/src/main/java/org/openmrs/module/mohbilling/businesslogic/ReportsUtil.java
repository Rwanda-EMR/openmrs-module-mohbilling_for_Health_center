			...
			if (!pi.getVoided() && !pi.getBillItem().getItemType().equals(2)) {
				if (categ.equals(pi.getBillItem().getHopService().getName())) {
					...
					if (insurancePolicy.getInsurance().getCurrentRate() != null) {
						...
						if((pi.getBillItem().getConsommation().getGlobalBill().getInsurance().getName().contains("MUTUELLE_CBHI_90%"))){
							if (pi.getBillItem().getHopService().getName().contains("TICKET MODERATEUR") || pi.getBillItem().getHopService().getName().contains("AUTRES")){
								paidAmountOnThisCategory = paidAmountOnThisCategory.add(paidQty.multiply(unitPrice));
							}
							if(pi.getBillItem().getHopService().getName().contains("AMBULANCE")){
								paidAmountOnThisCategory = paidAmountOnThisCategory.add(paidQty.multiply(unitPrice).multiply(new BigDecimal(0.1)));
							}
						}else {
...
//		List<HopService> reportColumns = GlobalPropertyConfig.getHospitalServiceByCategory(columns);
		...
		BigDecimal totalByDepartmentR =totalByDepartment.round(mc);
		...
		departRevenues.setAmount(totalByDepartmentR);
					...
					if (pi.getBillItem().getHopService().getName().equals(category) && pi.getBillItem().getConsommation().getThirdPartyBill() == null) {
						if(pi.getBillItem().getConsommation().getGlobalBill().getInsurance().getName().contains("MUTUELLE_CBHI_90%")){
							...
							if (pi.getBillItem().getHopService().getName().contains("AMBULANCE")){
								BigDecimal reqQty = pi.getBillItem().getQuantity();
								BigDecimal unitPrice = pi.getBillItem().getUnitPrice();
								totalByCategory = totalByCategory.add(reqQty.multiply(unitPrice).multiply(new BigDecimal(0.1)));
							}
						}else {
							BigDecimal reqQty = pi.getBillItem().getQuantity();
							BigDecimal unitPrice = pi.getBillItem().getUnitPrice();
